...
export class ExpressMetadataApplication {
  ...
  private constructor(private readonly container: Container) {
    ...
    controllers.forEach((controller) => {
      container.bind(CONTROLLER).to(controller as new (...args: never[]) => unknown);
    });

    // The way to get controller instances is through container
    const controllersFromContainer: NewableFunctionWithProperties[] =
      container.getAll<NewableFunctionWithProperties>(CONTROLLER);
    ...
    controllersFromContainer.forEach((controller: NewableFunctionWithProperties) => {
      ...
      methodsMetadata?.forEach((methodMetadata) => {
        ...
        const handler = this.createHandler(method.bind(controller), parameters);
   ...
   */
  public developmentOnly(handler: unknown): ExpressMetadataApplication {
  ...
  public use(handler: unknown): ExpressMetadataApplication {
    this.app.use(handler as Handler);
    return this;
  }
  ...
  public startListening(): void {
    ...
    this.app.listen(port, () => {
      console.log(
        `Running in ${this.isDevelopment ? 'development' : 'production'} mode on port ${port} 🤙`
      );
    });
  }
