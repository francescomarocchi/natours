...
export const params = (type: PARAMETER, parameterName?: string) => {
  return (target: { constructor: NewableFunction }, methodName: string | symbol, index: number): void => {
    ...
    if (!Reflect.hasOwnMetadata(CONTROLLER_METHOD_PARAMETER, target.constructor)) {
      ...
      metadataList = Reflect.getOwnMetadata(CONTROLLER_METHOD_PARAMETER, target.constructor) as ParametersMetadata;
...
export const getParametersMetadata = (ctor: NewableFunction): ParametersMetadata => {
  ...
  const methodMetadata: ParametersMetadata = Reflect.getOwnMetadata(CONTROLLER_METHOD_PARAMETER, ctor) as ParametersMetadata;
  // Prototype metadata
