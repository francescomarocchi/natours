...
export const controller = (path: string) => {
  return (target: NewableFunction): void => {
    ...
    Reflect.defineMetadata(CONTROLLER, [currentMetadata, ...previousMetadata], Reflect);
  };
};

export const getControllers = (): NewableFunction[] => {
  return (Reflect.getMetadata(CONTROLLER, Reflect) || []).map(
    (metadata: { target: NewableFunction }) => metadata.target as NewableFunction
  );
};

export const getControllerMetadata = (controller: NewableFunction): ControllerMetadata =>
  Reflect.getOwnMetadata(CONTROLLER, controller);
